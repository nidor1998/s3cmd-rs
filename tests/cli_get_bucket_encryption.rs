//! Process-level CLI tests for the `get-bucket-encryption` subcommand.
...
fn help_succeeds_and_lists_option_groups() {
    let (ok, stdout, _stderr, _code) = run(s7cmd().args(["get-bucket-encryption", "--help"]));
    assert!(ok, "get-bucket-encryption --help must succeed");
...
fn missing_target_exits_non_zero() {
    let (ok, _stdout, stderr, code) = run(s7cmd().arg("get-bucket-encryption"));
    assert!(!ok);
    assert_eq!(
        code,
        Some(2),
...
fn bucket_with_key_exits_1() {
    let (ok, _stdout, stderr, code) =
        run(s7cmd().args(["get-bucket-encryption", "s3://example/key"]));
    assert!(!ok);
    assert_eq!(
        code,
        Some(1),
...
fn mock_endpoint_missing_encryption_config_exits_4() {
    ...
    assert_eq!(
        code,
        Some(4),
        "missing encryption config should exit 4; stderr: {stderr}"
    );
    assert!(
        stderr.contains("encryption configuration for s3://mock-bucket not found"),

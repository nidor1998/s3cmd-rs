...
fn help_succeeds_and_lists_option_groups() {
    let (ok, stdout, _stderr, _code) = run(s7cmd().args(["get-public-access-block", "--help"]));
    assert!(ok, "get-public-access-block --help must succeed");
...
fn missing_target_exits_non_zero() {
    let (ok, _stdout, stderr, code) = run(s7cmd().arg("get-public-access-block"));
    assert!(!ok);
    assert_eq!(
        code,
        Some(2),
...
fn bucket_with_key_exits_1() {
    let (ok, _stdout, stderr, code) =
        run(s7cmd().args(["get-public-access-block", "s3://example/key"]));
    assert!(!ok);
    assert_eq!(
        code,
        Some(1),
